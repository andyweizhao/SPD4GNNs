...
@lru_cache(None)
def _sym_funcm_impl(func, x, **kwargs):
        
    func_name = func.__name__    
    func = partial(func, **kwargs)
    
    def _impl(x, func_name, delta=1e-3):
                
        x = sym(x)
        
        x = x + delta * torch.randn(x.shape, device=x.device)
        
        e_x, v_x = torch.linalg.eigh(x, UPLO='U')
        
        if func_name in ['log', 'pow', 'sqrt']: e_x = torch.clamp(e_x, min=1e-6) 

        out = v_x @ torch.diag_embed(func(e_x)) @ v_x.transpose(-1, -2)
        return out
    
    return _impl(x, func_name)
...
def sym_inv_sqrtm1(x: torch.Tensor) -> torch.Tensor:
    ...
    torch.Tensor
        :math:`x^{-1/2}`
    """
    return _sym_funcm_impl(torch.pow, x, exponent=-0.5)
